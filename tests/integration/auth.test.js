...
describe('Auth routes', () => {
  describe('POST /register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      const res = await request(app).post('/register').send(newUser).expect(httpStatus.CREATED);
    ...
    test('should return 400 error if email is invalid', async () => {
      ...
      await request(app).post('/register').send(newUser).expect(httpStatus.BAD_REQUEST);
    });
  ...
  describe('POST /login', () => {
    test('should return 200 and login user if email and password match', async () => {
      ...
      const res = await request(app).post('/login').send(loginCredentials).expect(httpStatus.OK);
    ...
    test('should return 401 error if there are no users with that email', async () => {
      ...
      const res = await request(app).post('/login').send(loginCredentials).expect(httpStatus.UNAUTHORIZED);
  ...
  describe('POST /logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      await request(app).post('/logout').send({ refreshToken }).expect(httpStatus.NO_CONTENT);
    ...
    test('should return 400 error if refresh token is missing from request body', async () => {
      await request(app).post('/logout').send().expect(httpStatus.BAD_REQUEST);
    });
    ...
    test('should return 404 error if refresh token is not found in the database', async () => {
      ...
      await request(app).post('/logout').send({ refreshToken }).expect(httpStatus.NOT_FOUND);
    });
  ...
  describe('POST /refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      const res = await request(app).post('/refresh-tokens').send({ refreshToken }).expect(httpStatus.OK);
    ...
    test('should return 400 error if refresh token is missing from request body', async () => {
      await request(app).post('/refresh-tokens').send().expect(httpStatus.BAD_REQUEST);
    });
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      ...
      await request(app).post('/refresh-tokens').send({ refreshToken }).expect(httpStatus.UNAUTHORIZED);
    });
